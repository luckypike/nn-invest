import React, { useContext } from 'react'
import { Route, Link } from 'react-router-dom'

import { Context } from '../Context'
import Sidebar from './Sidebar'

import styles from './News.module.css'

export default function News () {
  const { setOverlay } = useContext(Context)

  return (
    <div className={styles.root}>
      <div className={styles.title}>
        События
      </div>

      <div className={styles.news}>
        <Link to='/news/14' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            21 декабря 2020 г.
          </div>

          <div className={styles.text}>
            На инвестпортале Нижегородской области модернизирован личный кабинет инвестора
          </div>
        </Link>

        <Link to='/news/13' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            17 декабря 2020 г.
          </div>

          <div className={styles.text}>
            Нижегородская область вошла в топ-10 регионов с максимальным инвестиционным
            потенциалом в 2020 году
          </div>
        </Link>

        <Link to='/news/12' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            15 июля 2020 г.
          </div>

          <div className={styles.text}>
            Минэкономразвития РФ и Корпорация развития Нижегородской
            области подписали соглашение об управлении ОЭЗ «Кулибин»
          </div>
        </Link>

        <Link to='/news/11' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            9 июля 2020 г.
          </div>

          <div className={styles.text}>
            Глеб Никитин: «Вхождение Нижегородской области в топ-20 инвестиционного рейтинга — успех всего региона»
          </div>
        </Link>

        <Link to='/news/10' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            8 июля 2020 г.
          </div>

          <div className={styles.text}>
            Совет по земельным отношениям наделил проект по строительству научно-исследовательского лабораторного комплекса соцстатусом
          </div>
        </Link>

        <Link to='/news/9' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            7 июля 2020 г.
          </div>

          <div className={styles.text}>
            Правительство РФ поддержало расширение видов экономической деятельности для нижегородских ТОСЭР «Володарск» и «Решетиха»
          </div>
        </Link>

        <Link to='/news/8' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            4 июля 2020 г.
          </div>

          <div className={styles.text}>
            Корпорация развития Нижегородской области и компания «Сурбана Джуронг» подписали меморандум о намерениях
          </div>
        </Link>

        <Link to='/news/7' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            24 июня 2020 г.
          </div>

          <div className={styles.text}>
            Группа компаний «Рост» вложит в реализацию проекта «Тепличный комбинат «Нижегородский» 5,1 млрд рублей
          </div>
        </Link>

        <Link to='/news/6' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            1 июня 2020 г.
          </div>

          <div className={styles.text}>
            Компания «Синергетик» расширит свое производство на территории Нижегородской области
          </div>
        </Link>

        <Link to='/news/5' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            25 мая 2020 г.
          </div>

          <div className={styles.text}>
            «Особая экономическая зона «Кулибин» создана в Нижегородской области», - Глеб Никитин
          </div>
        </Link>

        <Link to='/news/4' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            10 мая 2020 г.
          </div>

          <div className={styles.text}>
            Корпорация развития сопровождает инвестпроект по строительству завода металлоизделий
          </div>
        </Link>

        <Link to='/news/3' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            6 мая 2020 г.
          </div>

          <div className={styles.text}>
            «Несмотря на сложную экономическую ситуацию, инвесторы заинтересованы в реализации своих проектов в регионе», - Тимур Халитов
          </div>
        </Link>

        <Link to='/news/1' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            10 апреля 2020 г.
          </div>

          <div className={styles.text}>
            Больше 7000 обращений бизнесменов поступило инвеступолномоченным за прошедшую неделю
          </div>
        </Link>

        <Link to='/news/2' className={styles.news_item} onClick={() => setOverlay(true)}>
          <div className={styles.date}>
            31 марта 2020 г.
          </div>

          <div className={styles.text}>
            Бизнес может обратиться за консультацией по мерам поддержки в связи с COVID-19 к инвеступолномоченным
          </div>
        </Link>
      </div>

      <Route
        path='/news/1'
        render={(props) =>
          <Sidebar
            {...props}
            title={<StairsTitle />}
            desc={<StairsDesc />}
            text={<StairsText />}
          />
        }
      />

      <Route
        path='/news/2'
        render={(props) =>
          <Sidebar
            {...props}
            title={<FairTitle />}
            desc={<FairDesc />}
            text={<FairText />}
          />
        }
      />

      <Route
        path='/news/3'
        render={(props) =>
          <Sidebar
            {...props}
            title={<CrisisTitle />}
            desc={<CrisisDesc />}
            text={<CrisisText />}
          />
        }
      />

      <Route
        path='/news/4'
        render={(props) =>
          <Sidebar
            {...props}
            title={<ForgingTitle />}
            text={<ForgingText />}
          />
        }
      />

      <Route
        path='/news/5'
        render={(props) =>
          <Sidebar
            {...props}
            title={<ZoneTitle />}
            text={<ZoneText />}
          />
        }
      />

      <Route
        path='/news/6'
        render={(props) =>
          <Sidebar
            {...props}
            title={<SynergyTitle />}
            desc={<SynergyDesc />}
            text={<SynergyText />}
          />
        }
      />

      <Route
        path='/news/7'
        render={(props) =>
          <Sidebar
            {...props}
            title={<GrowthTitle />}
            text={<GrowthText />}
          />
        }
      />

      <Route
        path='/news/8'
        render={(props) =>
          <Sidebar
            {...props}
            title={<MemoTitle />}
            desc={<MemoDesc />}
            text={<MemoText />}
          />
        }
      />

      <Route
        path='/news/9'
        render={(props) =>
          <Sidebar
            {...props}
            title={<ExtensionTitle />}
            desc={<ExtensionDesc />}
            text={<ExtensionText />}
          />
        }
      />

      <Route
        path='/news/10'
        render={(props) =>
          <Sidebar
            {...props}
            title={<StatusTitle />}
            desc={<StatusDesc />}
            text={<StatusText />}
          />
        }
      />

      <Route
        path='/news/11'
        render={(props) =>
          <Sidebar
            {...props}
            title={<TopTitle />}
            text={<TopText />}
          />
        }
      />

      <Route
        path='/news/12'
        render={(props) =>
          <Sidebar
            {...props}
            title={<AgreementTitle />}
            desc={<AgreementDesc />}
            text={<AgreementText />}
          />
        }
      />

      <Route
        path='/news/13'
        render={(props) =>
          <Sidebar
            {...props}
            title={<Top10Title />}
            desc={<Top10Desc />}
            text={<Top10Text />}
          />
        }
      />

      <Route
        path='/news/14'
        render={(props) =>
          <Sidebar
            {...props}
            title={<InvestportalTitle />}
            desc={<InvestportalDesc />}
            text={<InvestportalText />}
          />
        }
      />
    </div>
  )
}

function InvestportalTitle () {
  return (
    <div>
      На инвестпортале Нижегородской области модернизирован личный кабинет инвестора
    </div>
  )
}

function InvestportalDesc () {
  return (
    <div>
      Кроме того, обновлен функционал инвестиционной карты региона
    </div>
  )
}

function InvestportalText () {
  return (
    <div>
      <p>
        На инвестиционном портале Нижегородской области
        &nbsp;<a target="_blank" href="https://www.nn-invest.ru/">www.nn-invest.ru</a> обновлен
        функционал личного кабинета инвестора и инвестиционная карта региона.
      </p>

      <p>
        У инвестора теперь есть возможность в режиме онлайн просматривать данные об
        инициированных им проектах, следить за ходом их реализации, а также общаться с командой
        проекта посредством встроенного в систему общего чата, сигнализировать о возникающих
        проблемах. Для того чтобы получить доступ к функционалу, необходимо зарегистрироваться
        на инвестиционном портале в разделе «Личный кабинет инвестора».
      </p>

      <p>
        «Сервисы и контент инвестпортала максимально ориентированы на потенциальных инвесторов.
        Удобная навигация позволяет быстро найти на обновленном ресурсе информацию о преимуществах
        инвестирования в регион, о планируемых и реализуемых инвестпроектах, а также об имеющихся
        мерах поддержки», - отметил губернатор Нижегородской области <strong>Глеб Никитин</strong>.
      </p>

      <p>
        В рамках модернизации инвестиционной карты Нижегородской
        области <a target="_blank" href="https://newmap.nn-invest.ru/">https://newmap.nn-invest.ru/</a>
        также была внедрена новая система поиска фильтрации по типу участка, площади, муниципальному
        образованию, удаленности от областного центра и форме собственности. Сейчас инвестиционная
        карта представляет собой единую базу «зеленых» и «коричневых» инвестиционных площадок,
        которых на сегодняшний день более 600. В ней содержится вся необходимая информация по
        доступным участкам.
      </p>

      <p>
        «Инвестиционный портал является значимым инструментом в продвижении инвестиционного
        потенциала региона и электронной точкой входа для обращений инвесторов. По сути, он
        представляет собой один из важнейших каналов для прямой и обратной связи инвесторов
        и Корпорации развития Нижегородской области для оперативного решения возникающих
        проблемных вопросов. Поэтому мы постарались сделать его максимально удобным и
        функциональным», - подчеркнул генеральный директор Корпорации развития <strong>Тимур Халитов</strong>.
      </p>

      <p>
        Обновленный интерфейс портала адаптируется под различные устройства – запускать
        онлайн-сервисы и работать в личном кабинете можно с любого гаджета (компьютер, планшет, смартфон).
      </p>

      <p>
        Напомним, что для создания комфортных условий работы инвесторов по
        поручению <strong>Глеба Никитина</strong> в Нижегородской области созданы Корпорация
        развития, ОЭЗ «Кулибин» в Дзержинске, ТОСЭРы в Володарске и Решетихе, сформирован
        институт инвестиционных уполномоченных.
      </p>
    </div>
  )
}

function Top10Title () {
  return (
    <div>
      Нижегородская область вошла в топ-10 регионов с
      максимальным инвестиционным потенциалом в 2020 году
    </div>
  )
}

function Top10Desc () {
  return (
    <div>
      Регион сохранил позиции в рейтинге инвестпривлекательности с
      минимальными инвестиционными рисками
    </div>
  )
}

function Top10Text () {
  return (
    <div>
      <p>
        Нижегородская область в 2020 году сохранила свои позиции в группе 2А
        (группа с минимальными инвестиционными рисками) рейтинга инвестиционной
        привлекательности регионов России, а также третий раз вошла в топ-10 регионов
        с максимальным инвестиционным потенциалом. Об этом сообщил губернатор Нижегородской
        области <strong>Глеб Никитин</strong>.
      </p>

      <p>
        По его словам, таковы результаты исследований, проведенных агентством «РАЭКС Аналитика».
      </p>

      <p>
        «Мы уделяем серьезное внимание улучшению инвестиционного климата и развитию
        инфраструктуры региона. За прошедшее время было сделано очень многое. Введен институт
        инвестиционных уполномоченных, созданы две ТОСЭР, упрощен ряд административных процедур.
        В этом году создана особая экономическая зона «Кулибин». У нас даже сейчас, в условиях 
        пандемии, проходит немало встреч, видеоконференций с предпринимательским сообществом,
        где обсуждаются самые острые вопросы», — сказал губернатор Нижегородской области.
      </p>

      <p>
        «Несмотря на сложности этого года, мы сохраняем довольно высокую инвестиционную активность.
        За 9 месяцев объем инвестиций в основной капитал в регионе вырос на 2,9% по сравнению
        с аналогичным периодом прошлого года», - добавил <strong>Глеб Никитин</strong>.
      </p>

      <p>
        Напомним, рейтинг инвестиционной привлекательности регионов России традиционно
        строится на основе официальной информации Росстата и статистики федеральных ведомств:
        Минфина, Банка России, Минсвязи, МВД и Минприроды, а также на основе анкетирования
        представителей экспертного, инвестиционного и банковского сообществ.
      </p>

      <p>
        Ранее сообщалось, что Нижегородская область вошла в топ-20 регионов национального
        инвестиционного рейтинга АСИ в 2020 году. Регион поднялся на 39 позиций в сравнении
        с прошлым годом и занял 18-е место.
      </p>
    </div>
  )
}

function AgreementTitle () {
  return (
    <div>
      Минэкономразвития РФ и Корпорация развития Нижегородской
      области подписали соглашение об управлении ОЭЗ «Кулибин»
    </div>
  )
}

function AgreementDesc () {
  return (
    <div>
      Соглашение, в частности, закрепило за Корпорацией функции по поиску и
      привлечению резидентов в особую экономическую зону
    </div>
  )
}

function AgreementText () {
  return (
    <div>
      <p>
        Министерство экономического развития Российской Федерации и
        Корпорация развития Нижегородской области подписали соглашение об
        управлении особой экономической зоной «Кулибин», которая создана в
        Дзержинске. Подписи под документом поставили заместитель министра
        <strong>Сергей Галкин</strong> и генеральный директор Корпорации <strong>Тимур Халитов</strong>.
      </p>

      <p>
        Соглашение, в частности, закрепило за Корпорацией развития
        Нижегородской области в статусе управляющей компании ОЭЗ «Кулибин»
        функции по поиску и привлечению резидентов в особую экономическую
        зону.
      </p>

      <p>
        Также в соответствии с постановлением Правительства РФ №643 для
        оценки деятельности управляющей компании устанавливаются показатели
        эффективности функционирования ОЭЗ.
      </p>

      <p>
        «Одни из ключевых показателей – количество резидентов ОЭЗ;
        количество рабочих мест, созданных на ее территории, в том числе
        субъектами МСП; объем инвестиций, вложенный резидентами в проекты;
        объем используемых резидентами льгот. Эти показатели закрепляются за
        всеми управляющими компаниями особых экономических зон и позволяют
        на практике показать и доказать их эффективность», - подчеркнул <strong>Тимур
        Халитов</strong>.
      </p>

      <p>
        Функционирование ОЭЗ в Нижегородской области будет способствовать
        реализации задач, поставленных Президентом России <strong>Владимиром
        Путиным</strong> в рамках национальных проектов. В первую очередь, это касается
        производства импортозамещающих и экспортно-ориентированных товаров,
        повышения производительности труда.
      </p>

      <p>
        Напомним, что для создания комфортных условий работы инвесторов по
        поручению <strong>Глеба Никитина</strong> в Нижегородской области созданы Корпорация
        развития, ОЭЗ в Дзержинске, ТОСЭРы в Володарске и Решетихе,
        сформирован институт инвестиционных уполномоченных.
      </p>
    </div>
  )
}

function ZoneTitle () {
  return (
    <div>
      «Особая экономическая зона «Кулибин» создана в Нижегородской области», - Глеб Никитин
    </div>
  )
}

function ZoneText () {
  return (
    <div>
      <p>
        Особая экономическая зона (ОЭЗ) промышленно-производственного
        типа «Кулибин» в Дзержинске Нижегородской области одобрена
        правительством Российской Федерации. Постановление о ее создании
        подписал председатель Правительства Российской Федерации <strong>Михаил
        Мишустин</strong>. Об этом сообщил глава региона <strong>Глеб Никитин</strong>.
      </p>

      <p>
        Общая площадь ОЭЗ в Дзержинске, на территории промышленного
        предприятия АО «ДПО «Пластик»,превысит 72 га. Площадка имеет всю
        необходимую инфраструктуру, резерв энергетических мощностей, а также
        потенциал для увеличения территории особой экономической зоны еще на
        400 гектаров.
      </p>

      <p>
        «Это первая ОЭЗ в регионе. Теперь в области есть все основные
        российские особые экономические режимы, и мы можем на равных
        конкурировать с другими субъектами федерации за инвестиции. Создание
        ОЭЗ в Нижегородской конкурентоспособные условия ведения бизнеса по сравнению с регионами
        без нее. Так, преференциальные условия ОЭЗ «Кулибин» снизят для
        резидентов налоговые ставки на прибыль с 20% до 2% в первые 5 лет, до 5%
        - в следующие 5 лет и до 14,5% - в последующем, а также установят нулевые
        ставки по налогам на имущество, землю и транспортный налог.Заявка на
        создание особой экономической зоны была подготовлена Корпорацией
        развития региона, которая выступит ее управляющей компанией»,
        подчеркнул <strong>Глеб Никитин</strong>.
      </p>

      <p>
        Одним из приоритетных направлений деятельности станут химия и
        фармация. Именно поэтому территорией размещения ОЭЗ «Кулибин»
        выбран г.о.г.Дзержинск – один из крупнейших центров химической
        промышленности России.
      </p>

      <p>
        «У предприятий и научных организаций Дзержинска большой
        потенциал. Создание особой экономической зоны и развитие дзержинских
        индустриальных парков поможет его реализовать. Мы рассчитываем, что в
        ОЭЗ «Кулибин» появится не менее 15 новых производств, а это сотни
        дополнительных рабочих мест. Создание ОЭЗ также позволит расширить уже
        работающие в Дзержинске инновационные производства. Например,
        большие перспективы у технологий для рынка газомоторного топлива,
        созданных дзержинскими учеными дисперсий для строительной отрасли и
        других продуктов», - отметил первый заместитель губернатора
        Нижегородской области <strong>Евгений Люлин</strong>.
      </p>

      <p>
        «В Корпорацию уже поступают заявки от инвесторов, желающих
        открыть предприятие на территории ОЭЗ. Также на сегодняшний день у нас
        есть договоренности с тремя потенциальными инвесторами на сумму более 6
        млрд рублей. После проведения всех необходимых юридических процедур с
        ними будут заключены соглашения о сотрудничестве при реализации
        инвестиционного проекта на территории ОЭЗ», - отметил генеральный
        директор Корпорации развития Нижегородской области <strong>Тимур Халитов</strong>.
      </p>

      <p>
        Создание ОЭЗ в Нижегородской области будет способствовать
        реализации задач, поставленных президентом России <strong>Владимиром Путиным </strong>
        в рамках национальных проектов. В первую очередь это касается
        производства импортозамещающих и экспортно-ориентированных товаров,
        повышения производительности труда.
      </p>

      <p>
        Напомним, что для создания комфортных условий работы инвесторов по
        поручению <strong>Глеба Никитина</strong> в Нижегородской области созданы Корпорация
        развития, ТОСЭРы в Володарске и Решетихе, сформирован институт
        инвестиционных уполномоченных.
      </p>
    </div>
  )
}

function ForgingTitle () {
  return (
    <div>
      Корпорация развития сопровождает инвестпроект по строительству завода металлоизделий
    </div>
  )
}

function ForgingText () {
  return (
    <div>
      <p>
        Компания ООО «Первая ковка» планирует разместить на территории
        Нижегородской области завод по производству металлоизделий.
        Инвестиционный проект сопровождает Корпорация развития Нижегородской
        области.
      </p>

      <p>
        По словам генерального директора компании Елены Преображенской,
        завод будет специализироваться на производстве различных архитектурных
        элементов декора (балясин, перил, поручней, подставок, флюгеров и др.).
        Изготовление продукции будет осуществляться методом холодной обработки
        на станках итальянского производства.
      </p>

      <p>
        Планируемый объем выпуска составляет 1000 тонн металлоизделий в
        год. При полной загрузке проектируемое предприятие сможет выпускать
        примерно вдвое больший объем продукции.
      </p>

      <p>
        «Поставщиками сырья будут крупнооптовые коммерческие и торговые
        фирмы из России, Италии, стран ЕАЭС и Европейского Союза, а реализация
        готовой продукции будет проводиться на территории России и стран ЕАЭС,
        поэтому данный проект – один из ярких примеров, отвечающих направлениям
        нацпроекта «Международная кооперация и экспорт», – подчеркнул
        генеральный директор Корпорации развития Нижегородской области Тимур
        Халитов.
      </p>

      <p>
        Объем инвестиций в проект составит 50 млн рублей. К моменту ввода
        предприятия в эксплуатацию планируется создать 10 новых рабочих мест.
      </p>

      <p>
        Производство будет расположено в Нижнем Новгороде на ул.
        Коновалова. Компания планирует приступить к строительству во втором
        полугодии текущего года и завершить его в начале 2021 года.
      </p>
    </div>
  )
}

function CrisisTitle () {
  return (
    <div>
      «Несмотря на сложную экономическую ситуацию, инвесторы заинтересованы в реализации своих проектов в регионе», - Тимур Халитов
    </div>
  )
}

function CrisisDesc () {
  return (
    <div>
      Корпорация развития Нижегородской области на данный момент сопровождает 67 инвестпроектов. Об этом сообщил генеральный директор Корпорации Тимур Халитов.
    </div>
  )
}

function CrisisText () {
  return (
    <div>
      <p>
        Руководитель спецорганизации по привлечению инвестиций отметил, что, несмотря на сложную экономическую ситуацию, инвесторы заинтересованы в реализации своих проектов в регионе и не намерены от них отказываться. Те, кто уже подобрал земельный участок, оформляют необходимую разрешительную документацию, приступили к стадии проектирования или строительству.
      </p>

      <p>
        «Мы в постоянном контакте с инвесторами, это особенно важно в текущей ситуации. Безусловно, многие почувствовали на себе изменения, связанные с пандемией. У части предпринимателей в связи с эпидемией сроки реализации могут сдвинуться.  Сейчас сложно сказать на какой период, поскольку проблема не миновала. Но у инвесторов нет намерений затягивать реализацию. У тех, кто уже проанализировал текущее состояние дел, отсрочка не превышает 6 месяцев. Со своей стороны мы приложим все усилия и поможем инвесторам сократить сроки реализации проектов, решить возникшие проблемы, включая те, которые повлекла эпидемия коронавирусной инфекции», - подчеркнул <strong>Тимур Халитов</strong>.
      </p>

      <p>
        Генеральный директор Корпорации развития также отметил, что несмотря на сложную обстановку в стране и мире, в Корпорацию поступают запросы и от новых инвесторов, у которых реализация еще на стадии намерений. При этом для всех стараются подобрать максимально комфортные условия, чтобы бизнес любого уровня мог адаптироваться в новых реалиях.
      </p>

      <p>
        На сегодняшний день Корпорацией развития Нижегородской области ведётся работа по 67 инвестиционным проектам на сумму порядка 150 млрд рублей. Эти проекты предполагают создание более 7,5 тысяч новых рабочих мест.
      </p>

      <p>
        Напомним, инвесторы могут обратиться за консультацией к инвеступолномоченным, а также на горячую линию поддержки: 8 (800) 350-12-52.
      </p>
    </div>
  )
}

function StairsTitle () {
  return (
    <div>
      Больше 7000 обращений бизнесменов поступило инвеступолномоченным за прошедшую неделю
    </div>
  )
}

function StairsDesc () {
  return (
    <div>
      Более 7000 обращений от предпринимателей поступило инвестиционным
      уполномоченным муниципалитетов Нижегородской области, а также на
      горячую линию поддержки инвесторов по вопросам, связанным с
      эпидемиологической обстановкой. Об этом сообщил генеральный директор
      Корпорации развития Нижегородской области Тимур Халитов.
    </div>
  )
}

function StairsText () {
  return (
    <div>
      <p>
        «Многие запросы касаются мер поддержки, среди которых предоставление
        налоговых каникул для предприятий и ИП и возможность участия в кредитных
        программах для выплаты зарплаты сотрудникам» - отметил <strong>Тимур Халитов</strong>
      </p>

      <p>
        «У бизнеса сейчас много вопросов. При этом большая часть вопросов именно к
        инвеступолномоченным, как к представителям муниципалитетов, касается
        получений подтверждений на право работать в нерабочие дни. В этой связи одна из
        основных задач на сегодня для получивших Подтверждение предпринимателей –
        строгое соблюдение п. 8 Указа Губернатора «О введении режима повышенной
        готовности», который включает особые условия допуска к работе», - подчеркнул
        губернатор Нижегородской области <strong>Глеб Никитин</strong>.
      </p>

      <p>
        Напомним, на всей территории региона разрешена работа организаций, которые
        не подпадают под действие Указа Президента РФ No 239, а также
        системообразующих предприятий, утвержденных на федеральном уровне и на
        уровне Нижегородской области и их контрагентов. Они должны будут получить
        подтверждение на право работать в нерабочие дни в региональных органах
        исполнительной власти.
      </p>
      <p>
        Также работать в нерабочие дни на всей территории региона могут
        организации, осуществляющие деятельность в таких сферах, как: сельское
        хозяйство, ремонт автотранспортных средств, пассажирские перевозки,
        производственные предприятия, осуществляющие деятельность на обособленной
        огороженной территории, при условии соблюдения работниками дистанции в 1,5
        метра. Подтверждения на право работать в нерабочие дни для этих организаций
        выдают органы местного самоуправления.
      </p>
    </div>
  )
}

function FairTitle () {
  return (
    <div>
      Бизнес может обратиться за консультацией по мерам поддержки в связи с COVID-19 к инвеступолномоченным
    </div>
  )
}

function FairDesc () {
  return (
    <div>
      Инвесторы смогут обратиться за консультацией по мерам поддержки,
      связанным с эпидемиологической обстановкой, к инвеступолномоченным от
      муниципальных образований Нижегородской области.
    </div>
  )
}

function FairText () {
  return (
    <div>
      <p>
        Институт инвестиционных уполномоченных был сформирован в регионе
        по поручению губернатора <strong>Глеба Никитина</strong> в 2019 году. Координатором
        работы инвеступолномоченных выступает Корпорация развития
        Нижегородской области.
      </p>

      <p>
        Всего назначены 82 инвестиционных
        уполномоченных от ОМСУ, органов исполнительной власти,
        ресурсоснабжающих и инфраструктурных компаний региона. Большинство
        из них – 52 – представители муниципалитетов в должности не ниже
        заместителя главы администрации. Целью работы уполномоченных является
        организация эффективного содействия в реализации инвестпроектов на
        местах.
      </p>

      <p>
        <strong>Глеб Никитин</strong> подчеркнул, что в текущей ситуации особо важен
        прямой выход инвесторов на инвестиционных уполномоченных <em>(список
        инвеступолномоченных с контактами – в приложении)</em>.
      </p>

      <p>
        «У инвесторов, предпринимателей сейчас много вопросов, и они не
        должны оставаться с ними один на один. Прошу инвестиционных
        уполномоченных и ответственные органы максимально оперативно
        реагировать на запросы», - отметил губернатор.
      </p>

      <p>
        Кроме того, получить консультацию предприниматели могут,
        обратившись на <strong>горячую линию поддержки инвесторов</strong>: 8 (800) 350-12-52.
      </p>

      <p>
        «Специалисты Корпорации развития ответят на вопросы инвесторов и
        дадут все необходимые разъяснения по мерам поддержки, которые были
        приняты в условиях сложившейся эпидемиологической обстановки», -
        подчеркнул генеральный директор Корпорации развития Нижегородской
        области <strong>Тимур Халитов</strong>.
      </p>

      <p>
        Напомним, что в дистанционном режиме продолжает свою работу центр
        «Мой бизнес», открывшийся в рамках нацпроекта «Малое и среднее
        предпринимательство и поддержка индивидуальной предпринимательской
        инициативы». С 18 марта 2020 года на портале мойбизнес52.рф заработала
        отдельная кнопка: «Обращение, связанное с коронавирусом». Нижегородские
        предприниматели также могут сообщить о связанных с коронавирусом
        проблемах, позвонив по телефону горячей линии «Мой бизнес»:
        8−800−301−29−94. Кроме того, в регионе начал действовать дополнительный
        номер телефона: +7 951 911 7002, по которому в viber-чате представители
        бизнеса могут получить оперативную консультацию в условиях риска
        распространения коронавируса.
      </p>
    </div>
  )
}

function SynergyTitle () {
  return (
    <div>
      Компания «Синергетик» расширит свое производство на территории Нижегородской области
    </div>
  )
}

function SynergyDesc () {
  return (
    <div>
      Проект сопровождает Корпорация развития Нижегородской области
    </div>
  )
}

function SynergyText () {
  return (
    <div>
      <p>
        Компания «Синергетик» планирует расширить свое производство и разместить на территории Нижегородской области производственно-лабораторный корпус по выпуску товаров бытовой химии. Инвестиционный проект сопровождает Корпорация развития Нижегородской области.
      </p>

      <p>
        По словам генерального директора компании Алексея Зюзина, новый производственно-лабораторный комплекс будет специализироваться на производстве экологичных моющих средств под торговой маркой SYNERGETIC. Его строительство позволит увеличить ассортимент и расширить линейки выпускаемой продукции, в том числе антисептических средств, производимых компанией.
      </p>

      <p>
        Планируемый объем выпуска продукции на территории нового корпуса составит 10 000 тонн в год.
      </p>

      <p>
        В ближайшее время компания планирует активное продвижение продукции на международные рынки - как в страны Таможенного союза, так и за его пределами.
      </p>

      <p>
        «Производство предполагается разместить на территории г.о.г.Дзержинск, в непосредственной близости к действующему заводу компании», - подчеркнул генеральный директор Корпорации развития Нижегородской области Тимур Халитов.
      </p>

      <p>
        Объем инвестиций в проект составит 50 млн рублей. К моменту ввода предприятия в эксплуатацию планируется создать 30 новых рабочих мест. Срок реализации проекта – 2 года. Компания планирует приступить к строительству в конце 2020 года.
      </p>
    </div>
  )
}

function GrowthTitle () {
  return (
    <div>
      «Группа компаний «Рост» вложит в реализацию проекта «Тепличный комбинат «Нижегородский» 5,1 млрд рублей
    </div>
  )
}

function GrowthText () {
  return (
    <div>
      <p>
        Увеличить производство овощей закрытого грунта почти в два раза позволит строительство инновационного «Тепличного комбината «Нижегородский» в городе Бор. Объем инвестиций группы компаний «РОСТ» в проект – 5,1 млрд рублей. Реализация этого проекта позволит создать 250 новых рабочих мест.
      </p>

      <p>
        Строительство крупнейшего в регионе тепличного комплекса в г. Бор планируют завершить в конце этого года. Инвестиционный проект сопровождает Корпорация развития.
      </p>

      <p>
        Общая площадь «Тепличного комбината «Нижегородский» составит 17,3 га. В теплицах будет использоваться современная технология досветки и автоматизированный контроль микроклимата и роста растений, что обеспечит среднегодовую урожайность на уровне 70 кг/м2 для томатов и 140 кг/м2 для огурцов. Предполагаемый валовый сбор конечной продукции при выходе на полную мощность составит 14,1 тыс. тонн огурцов и 3,5 тыс. тонн томатов в год.
      </p>

      <p>
        «Мы обсудили с руководством предприятия перспективы расширения проекта. Инвестор рассмотрит возможность строительства еще одного крупного тепличного комплекса», - подчеркнул первый заместитель губернатора Нижегородской области Евгений Люлин.
      </p>

      <p>
        «Мы оказываем компании содействие в оформлении различной документации, чтобы ускорить строительство. Кроме того, инвестор подал заявку на присвоение проекту статуса приоритетного инвестиционного, что позволит получить ряд льгот», - сообщил генеральный директор Корпорации Тимур Халитов.
      </p>
    </div>
  )
}

function MemoTitle () {
  return (
    <div>
      Корпорация развития Нижегородской области и компания «Сурбана Джуронг» подписали меморандум о намерениях
    </div>
  )
}

function MemoDesc () {
  return (
    <div>
      Соглашение подписали генеральный директор компании «Сурбана Джуронг» Чун Чонг Йео и генеральный директор АО «Корпорация развития Нижегородской области» Тимур Халитов.
    </div>
  )
}

function MemoText () {
  return (
    <div>
      <p>
        Документ предусматривает совместную работу по разработке концепции комплексного развития территорий Нижегородской области, включая территории промышленных парков, научных центров, создаваемых Корпорацией и правительством Нижегородской области, что позволит создать новые рабочие места и повысить благоприятность инвестиционного и туристического климата Нижегородской области.
      </p>

      <p>
        «Основным направлением деятельности компании является строительство и развитие инфраструктуры Сингапура, а также реализация проектов на международном уровне. Так «Сурбана Джуронг» участвовала в генеральном планировании инфраструктуры промышленного парка и глубоководного порта в Мьянме. Поэтому мы видим хорошие перспективы сотрудничества с компанией на уровне нижегородского региона», - отметил Тимур Халитов.
      </p>

      <p>
        Меморандум закрепляет договоренности, достигнутые в рамках встречи губернатора Нижегородской области Глеба Никитина с представителями компании в ходе бизнес-миссии в Малайзию и Сингапур в ноябре 2019 года.
      </p>

      <p>
        Справка. Сурбана Джуронг, Лтд. – сингапурская государственная консалтинговая компания, специализирующаяся на инфраструктуре и развитии городских территорий. Она была образована 22 июня 2015 года в результате слияния двух компаний: ДжиТиСи Корпорейшн (JTC Corporation) и Темасек Холдингс (Temasek Holdings). Компания принимала участие в реализации проектов по развитию территорий в Псковской области, Приморском и Краснодарском краях.
      </p>
    </div>
  )
}

function ExtensionTitle () {
  return (
    <div>
      Правительство РФ поддержало расширение видов экономической деятельности для нижегородских ТОСЭР «Володарск» и «Решетиха»
    </div>
  )
}

function ExtensionDesc () {
  return (
    <div>
      Постановление правительства РФ — №972 «О внесении изменений в отдельные акты» подписал премьер-министр РФ Михаил Мишустин.
    </div>
  )
}

function ExtensionText () {
  return (
    <div>
      <p>
        Нижегородскими территориями со льготными режимами интересуются инвесторы из целого ряда сфер, которые не попадали в действующие до сегодняшнего дня перечни. Губернатор Нижегородской области Глеб Никитин поблагодарил федеральные органы власти за принятое решение.
      </p>

      <p>
        «Теперь резидентами наших ТОСЭР могут быть производители одежды, красок и лаков, готовых металлических изделий, электрического оборудования и другой продукции. Они могут рассчитывать на преференции в виде освобождения от налогов на землю и имущество, а также снижения ставки по налогу на прибыль», — отметил Глеб Никитин.
      </p>

      <p>
        Нижегородские ТОСЭР имеют развитую инфраструктуру электроснабжения, теплоснабжения, водоснабжения и водоотведения, мощности которой используются не полностью.
      </p>

      <p>
        Ставка налога на прибыль для резидентов будет составлять 5% в первые пять лет и 12−13% - в следующие пять. Кроме того, ставка по страховым взносам будет снижена до 7,6%.
      </p>
    </div>
  )
}

function StatusTitle () {
  return (
    <div>
      Совет по земельным отношениям наделил проект по строительству научно-исследовательского лабораторного комплекса соцстатусом
    </div>
  )
}

function StatusDesc () {
  return (
    <div>
      Статус предполагает освобождение от взносов за коммунальную инфраструктуру
    </div>
  )
}

function StatusText () {
  return (
    <div>
      <p>
        Совет по земельным и имущественным отношениям при правительстве Нижегородской области наделил проект по строительству научно-исследовательского лабораторного комплекса в Нижнем Новгороде статусом социального 8 июля 2020 года.
      </p>

      <p>
        Статус присвоен проекту ООО «ТК «Нефтепроминвест» по строительству научно - исследовательского лабораторного комплекса. Он будет включать в себя опытно-промышленное производство высокочастотных химических соединений, используемых в микроэлектронике. Данные химические соединения являются сложными продуктами высокой и сверхвысокой чистоты, применяемыми при нанесении тонких нано-пленок с использованием технологий CVD и ALD. Эти технологии, в свою очередь, используются на всех современных крупных производствах полупроводников, микросхем, процессоров, чипов, без которых не обходится ни одно современное электронное устройство: смартфоны, компьютеры, дисплеи.
      </p>

      <p>
        «В процессе реализации проекта к работе в нашей компании будут привлекаться в том числе молодые специалисты из Института химии при ННГУ им. Лобачевского, которым будет обеспечен полный социальный пакет, а также обучение и повышение квалификации в соответствии с международными стандартами за счет компании», - отметил генеральный директор ТК «Нефтепроминвест» Илья Илюшин.
      </p>

      <p>
        ООО «ТК «Нефтепроминвест» планирует ввести объект в эксплуатацию в 2021 году. Объем инвестиций – 240 млн рублей, площадь участка – порядка 30 000 кв. метров. Благодаря реализации данного проекта будет создано порядка 60 новых рабочих мест.
      </p>

      <p>
        Социальный статус предполагает предоставление участка без торгов и освобождение от взносов за коммунальную инфраструктуру. Ранее совет выделил компании участок в Сормовском районе Нижнего Новгорода, что позволит обеспечить к ней удобный доступ необходимого технического, инженерного и научного персонала, а также близость к научным центрам.
      </p>

      <p>
        Проект реализуется при поддержке Корпорации развития Нижегородской области.
      </p>

      <p>
        «Это уникальное производство, на котором порядка 50% персонала будут составлять высококвалифицированные и высокооплачиваемые инженеры-химики, в основном со степенью кандидатов и докторов наук. Потенциальными клиентами продукции будут являться такие крупные мировые производители полупроводников из Азии, Европы и США как TSMC, Qualcomm, SK Hynix, UMC, Toshiba, Infineon, Avago. Для нас важно, что данный инвестпроект отвечает задачам сразу двух нацпроектов - «Международная кооперация и экспорт» и «Наука», - подчеркнул генеральный директор Корпорации развития Тимур Халитов.
      </p>

      <p>
        Также одобрена заявка ООО «Синергетик» на выделение участка в Дзержинске под строительство производственно-лабораторного корпуса по выпуску товаров бытовой химии (площадь участка – 7 560 кв м, объем инвестиций – 50 млн рублей, срок строительства – 2 года). Планируемый объем выпуска продукции на территории нового корпуса составит 10 000 тонн в год. Проект также сопровождает Корпорация развития региона.
      </p>

      <p>
        Напомним, что для создания комфортных условий работы инвесторов по поручению губернатора Глеба Никитина в Нижегородской области созданы Корпорация развития, ОЭЗ в Дзержинске, ТОСЭР в Володарске и Решетихе, сформирован институт инвестиционных уполномоченных.
      </p>
    </div>
  )
}

function TopTitle () {
  return (
    <div>
      Глеб Никитин: «Вхождение Нижегородской области в топ-20 инвестиционного рейтинга — успех всего региона»
    </div>
  )
}

function TopText () {
  return (
    <div>
      <p>
        Нижегородская область вошла в топ-20 регионов национального инвестиционного рейтинга-2020. Регион поднялся на 39 позиций в сравнении с прошлым годом — на 18-е место. Об итогах рейтинга Президенту России Владимиру Путину рассказала глава АСИ Светлана Чупшева 9 июля 2020 года на заседании наблюдательного совета агентства.
      </p>

      <p>
        Как отметил губернатор Нижегородской области Глеб Никитин, вхождение в топ-20 инвестиционного рейтинга — успех всего региона.
      </p>

      <p>
        «Всего за год Нижегородской области удалось преодолеть в рейтинге почти 40 позиций и войти в двадцатку лучших. Этому предшествовала большая работа по улучшению инвестиционного климата. Для меня очень важно, что при формировании рейтинга учитываются абсолютно четкие и понятные критерии, совпадающие с мнением предпринимательского сообщества. Хочется поблагодарить команду Агентства стратегических инициатив за методическую поддержку, которая позволила нам принимать управленческие решения в интересах инвесторов», — сказал Глеб Никитин.
      </p>

      <p>
        «За прошедшее время было сделано очень много. Введен институт инвестиционных уполномоченных, созданы две ТОСЭР, упрощен ряд административных процедур. В этом году стартовало создание особой экономической зоны „Кулибин“. Я лично регулярно встречаюсь с предпринимательским сообществом, обсуждаю с ними острые темы. Инвестиционный рейтинг — важный сигнал для инвесторов о том, что в Нижегородской области можно и нужно работать», — добавил глава региона.
      </p>

      <p>
        Рейтинг оценивает усилия региональных управленческих команд по созданию благоприятных условий ведения бизнеса и рассчитывается на основе опросов предпринимателей. Он включает такие направления оценки, как скорость и легкость административных процедур (разрешения на строительство и регистрации права собственности), наличие и качество инфраструктуры и ресурсов, работа институтов для бизнеса и поддержка малого бизнеса.
      </p>

      <p>
        «В этом году половина регионов улучшили свой интегральный индекс по сравнению с прошлым годом, 31 из них демонстрируют устойчивый рост второй год подряд», — отметила генеральный директор АСИ Светлана Чупшева.
      </p>
    </div>
  )
}
